  ...
  if (!state?.enabled) {
    return (
      <div className="rounded-2xl border border-klik-line bg-klik-card p-4 md:p-6">
        ...
        <h2 className="mt-1 font-display text-xl font-bold">Retiro manual por ahora</h2>
        <p className="mt-3 text-sm leading-6 text-white/60">
          El comprador paga con tu Stripe de plataforma. Nosotros llevamos la cuenta de quién le toca
          (85/10/5) y, cuando pides retiro, lo depositamos a mano. Para automatizarlo, activa Stripe
          Connect en el dashboard de Stripe y pon{" "}
          <code className="text-klik-cyan">STRIPE_CONNECT_ENABLED=true</code> en Vercel.
        </p>
      </div>
    );
  }
