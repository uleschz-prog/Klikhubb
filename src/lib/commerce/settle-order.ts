    ...
    if (existingEnrollment) {
      const updated = await tx.enrollment.update({
        ...
        data: {
          status: "ACTIVE",
          orderId: order.id,
          ...(!input.renewal && existingEnrollment.status !== "ACTIVE" ? { progressPct: 0 } : {}),
        },
      });
    ...
    for (const line of lines) {
      ...
      const commission = await tx.commission.create({
        data: {
          ...
          type: line.type === "UNILEVEL" ? CommissionType.UNILEVEL : CommissionType.CREATOR_SALE,
          level: line.level,
      ...
      await creditWallet(tx, {
        ...
        cents: line.amountCents,
        type: line.type === "UNILEVEL" ? LedgerType.COMMISSION : LedgerType.SALE,
        destination: "pending",
        orderId: order.id,
