# Qlyk — Versiones cortas para WhatsApp

Copia y pega. Cada bloque cabe en 1–2 mensajes.

---

## 1. Presentación general (para cualquier contacto)

```
🎬 *Qlyk* — del video al pago, sin salir del feed.

Red social donde publicas, vendes y cobras en el mismo sitio:
✅ Feed de video
✅ Cursos y membresías
✅ Comunidad
✅ Monedero integrado

Registro *gratis* y directo 👇
https://qlyk.vercel.app
```

---

## 2. Para creadores (manual corto)

```
*¿Eres creador? Así funciona Qlyk* 🚀

1️⃣ Creas cuenta gratis → https://qlyk.vercel.app/register
2️⃣ Subes video con tu oferta (/publish) o armes un curso (/studio)
3️⃣ Tu gente compra *desde el feed* con tarjeta o SPEI
4️⃣ Cobras en tu monedero

*¿Cuánto te quedas?*
• Sin referidor: *90%* tuyo · 10% plataforma
• Con referidor: *85%* tuyo · 5% quien invitó · 10% plataforma

El dinero se libera a los *14 días* (retención anti-fraude).
Retiro mínimo: *10 USD*.

*Invita, no reclutes:* ganas 5% cuando alguien que invitaste *compra*, no por registrarse.

Tu link con código:
https://qlyk.vercel.app?ref=TUCODIGO

¿Dudas? Escríbenos 💬
```

---

## 3. Para inversionistas (pitch corto)

```
*Qlyk — oportunidad en 30 segundos* 📊

*Problema:* creadores repartidos entre TikTok + Gumroad + Skool. Pierden conversión en cada enlace.

*Solución:* una app donde *video → compra → curso → comunidad* en un solo clic.

*Modelo:* 10% de cada venta (take rate claro).
Reparto: 85% creador · 10% plataforma · 5% referido (1 solo nivel).
*No es MLM.* Sin pago por reclutar.

*Estado:* producto live, pagos Stripe + SPEI, Course Studio, wallet, legal RGPD.
*URL:* https://qlyk.vercel.app

Buscamos capital para growth de creadores ancla y escala LATAM/hispano.

¿Te interesa una call de 15 min? 📞
```

---

## 4. Invitación con referido (personalizable)

```
Te invito a *Qlyk* 🎬

Publica video, vende cursos y cobra sin mandar a tu gente a otro link.

Cuenta gratis aquí 👇
https://qlyk.vercel.app?ref=TUCODIGO

Del video al pago. Sin salir del feed.
```

---

## 5. Recordatorio post-registro (creador)

```
*Primeros pasos en Qlyk* ✅

1. Entra a tu dashboard: /dashboard
2. Crea tu primera oferta en /studio o sube un video en /publish
3. Comparte tu feed y tu link con ?ref=TUCODIGO
4. Revisa tu monedero en /wallet

Manual completo en el repo: docs/manual-creadores.md
Plataforma: https://qlyk.vercel.app
```

---

*Sustituye TUCODIGO por tu código real de referido.*
